...
public class NotificationsController {
    ...
    private String getStringForMessage(MessageObject messageObject, boolean shortMessage) {
        ...
        String msg = null;
        SharedPreferences mainPref = ApplicationLoader.applicationContext.getSharedPreferences("mainconfig", Activity.MODE_PRIVATE);
        boolean showPreview = mainPref.getBoolean("when_passcode_message_preview", false);
        boolean showMessage = (AndroidUtilities.needShowPasscode(false) || UserConfig.isWaitingForPasscodeEnter);
        if ((int)dialog_id == 0) {
            msg = LocaleController.getString("YouHaveNewMessage", R.string.YouHaveNewMessage);
        } else if ((showMessage == true) && (showPreview == false)) {
            msg = LocaleController.getString("YouHaveNewMessage", R.string.YouHaveNewMessage);
        } else {
    ...
    private void showOrUpdateNotification(boolean notifyAboutLast) {
        ...
        try {
            ...
            if (pushMessages.size() == 1) {
                ...
                // FIXME: Incoming Message Count
                int count = Math.min(1, pushMessages.size());
    ...
    public void showExtraNotifications(NotificationCompat.Builder notificationBuilder, boolean notifyAboutLast) {
        ...
        for (long dialog_id : sortedDialogs) {
            ...
            if (chat == null && user != null && user.phone != null && user.phone.length() > 0) {
                builder.addPerson("tel:" + ExtraUtils.getPhoneNumber(user.phone));
            }
