...
public class MessageObject {
    ...
    public MessageObject(TLRPC.Message message, AbstractMap<Integer, TLRPC.User> users, boolean generateLayout) {
                ...
                if (message.action instanceof TLRPC.TL_messageActionChatCreate) {
                    ...
                    if (message.action.ttl != 0) {
                        if (isOut()) {
                            ...
                            if (fromUser != null) {
                                messageText = LocaleController.formatString("MessageLifetimeChanged", R.string.MessageLifetimeChanged, ContactsController.formatName(fromUser.first_name, fromUser.last_name), AndroidUtilities.formatTTLString(message.action.ttl));
                            } else {
                        ...
                        if (isOut()) {
                            ...
                            if (fromUser != null) {
                                messageText = LocaleController.formatString("MessageLifetimeRemoved", R.string.MessageLifetimeRemoved, ContactsController.formatName(fromUser.first_name, fromUser.last_name));
                            } else {
                    ...
                    String name = "";
                    if (to_user != null) {
                        name = ContactsController.formatName(to_user.first_name, to_user.last_name);
                    }
                    ...
                    if (message.action.encryptedAction instanceof TLRPC.TL_decryptedMessageActionScreenshotMessages) {
                        ...
                        if (action.ttl_seconds != 0) {
                            if (isOut()) {
                                ...
                                if (fromUser != null) {
                                    messageText = LocaleController.formatString("MessageLifetimeChanged", R.string.MessageLifetimeChanged, ContactsController.formatName(fromUser.first_name, fromUser.last_name), AndroidUtilities.formatTTLString(action.ttl_seconds));
                                } else {
        ...
        TextPaint bigTextPaint = new TextPaint(Paint.ANTI_ALIAS_FLAG);
        int textSize = AndroidUtilities.dp(18);
        if(Emoji.checkEmojiCount(messageText) == 1 && messageText.length() <= 2){
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
                textSize = AndroidUtilities.dp(40);
            }else{
                textSize = AndroidUtilities.dp(30);
            }
        }
        bigTextPaint.setColor(0xff000000);
        bigTextPaint.setTextSize(textSize);
        //messageText = Emoji.replaceEmoji(messageText, bigTextPaint.getFontMetricsInt(), textSize);
        if(messageText != null) {
            SpannableStringBuilder builder = new SpannableStringBuilder(messageText);
            EmojiconHandler.addEmojis(ApplicationLoader.applicationContext, builder, textSize, bigTextPaint.getFontMetricsInt(), AndroidUtilities.density);
            messageText = builder;
        }
    ...
    public void generateCaption() {
        ...
        if (messageOwner.media != null && messageOwner.media.caption != null && messageOwner.media.caption.length() > 0) {
            SpannableStringBuilder builder = new SpannableStringBuilder(messageOwner.media.caption);
            EmojiconHandler.addEmojis(ApplicationLoader.applicationContext, builder, AndroidUtilities.dp(20), textPaint.getFontMetricsInt(), AndroidUtilities.density);
            caption = builder;
