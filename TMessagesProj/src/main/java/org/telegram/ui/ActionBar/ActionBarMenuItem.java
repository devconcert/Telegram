...
public class ActionBarMenuItem extends FrameLayoutFixed {
    ...
    public ActionBarMenuItem(Context context, ActionBarMenu menu, Drawable background) {
        super(context);
    ...
    private void setRemoteTheme() {
        Drawable drawable = ThemeManager.getInstance().getRemoteResourceDrawable("search_light_states");
        if (drawable != null) {
            if (Build.VERSION.SDK_INT >= 16) {
                searchField.setBackground(drawable);
            } else {
                searchField.setBackgroundDrawable(drawable);
            }
        }
        int color = ThemeManager.getInstance().getRemoteResourceColor("search_text");
        if(color != -1){
            searchField.setTextColor(color);
        }else{
            searchField.setTextColor(ApplicationLoader.applicationContext.getResources().getColor(R.color.white));
        }
    }
