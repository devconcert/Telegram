...
public class ChatBaseCell extends BaseCell {
    ...
    public ChatBaseCell(Context context) {
        ...
        if (timePaintIn == null) {
            ...
            int timeIn = ThemeManager.getInstance().getRemoteResourceColor("chat_time_in_text");
            if(timeIn != -1) timePaintIn.setColor(timeIn);
            else timePaintIn.setColor(0xffa1aab3);

            timePaintOut = new TextPaint(TextPaint.ANTI_ALIAS_FLAG);
            timePaintOut.setTextSize(AndroidUtilities.dp(12));
            int timeOut = ThemeManager.getInstance().getRemoteResourceColor("chat_time_out_text");
            if(timeOut != -1) timePaintOut.setColor(timeOut);
            else timePaintOut.setColor(0xff70b15c);

            timeMediaPaint = new TextPaint(TextPaint.ANTI_ALIAS_FLAG);
            timeMediaPaint.setTextSize(AndroidUtilities.dp(12));
            int timeMedia = ThemeManager.getInstance().getRemoteResourceColor("chat_time_media_text");
            if(timeMedia != -1) timeMediaPaint.setColor(timeMedia);
            else timeMediaPaint.setColor(0xffffffff);
        ...
        if (messageObject.isReply()) {
            ...
            if (messageObject.replyMessageObject != null) {
                ...
                if (messageObject.replyMessageObject.messageText != null && messageObject.replyMessageObject.messageText.length() > 0) {
                    ...
                    EmojiconHandler.addEmojis(ApplicationLoader.applicationContext, builder, AndroidUtilities.dp(14), replyTextPaint.getFontMetricsInt(), AndroidUtilities.density);
                    stringFinalText = builder;
